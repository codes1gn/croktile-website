---
title: "Integers and i-tuples"
description: "Integer and integer-tuple types for control flow, tiling factors, and operations with mdspan."
order: 4
---

## Overview
In this section, you will learn how to define *integer*s and integer-tuples (*i-tuple*s) in Croktile code and understand their usage.
...
However, since Croktile can infer the type from the *initialization expression*, you can also define an integer variable in another style:
...
In Croktile, the *mdspan*-typed shape can not be used as the function parameter. However, integers can be passed from the host to the Tileflow program, and from the Tileflow program to the device:
...
Since Croktile can infer types from the *initialization expression*, programmers can often omit the `ituple` keyword. However, without explicit type annotation, an *ituple* variable definition might look similar to an *mdspan* definition if you are not yet familiar with Croktile. To distinguish them:
...
*I-tuples* are not frequently used in real Croktile code. However, their variants, **bounded i-tuple**s, which bind an *i-tuple* with an *mdspan* to indicate its possible range, are essential for constructing Croktile loops. We will cover *bounded i-tuples* in more detail in later chapters.

## Quick Summary
In this section, we learned how to define *integer*s and *i-tuple*s in Croktile. The *Integer*s follow C/C++ syntax. *i-tuples* follow a syntax similar to *mdspan*, but they can operate on *mdspan* to derive new values.
