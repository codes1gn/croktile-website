---
title: "Spanned data"
description: "Declaring spanned buffers, fundamental types, storage specifiers, parameters, and lifetime."
order: 5
---

## Overview
Croktile *tileflow program* describes how to move data around. Therefore, declaring or defining the data or buffer are fundamental. In this section, you will learn related syntax.

## *Spanned*: Data and Buffers
The primary focus of a Tileflow program is to manipulate large datasets by moving them around. In some terminology, the input and output of a Croktile function are classified as "input data" and "output data," respectively. Meanwhile, any other storage is referred to as "buffers." This conceptually distinguishes between "external" and "internal" memory from a function's perspective. However, both "data" and "buffer" refer to storage positions. In Croktile, both are typed as **spanned**, reflecting that they are data/buffers associated with an
...
In Croktile terminology, the element type is referred to as the **fundamental type**. Croktile supports multiple fundamental types, including:
...
Note `s32` is different from `int`. Although they occupy the same amount of storage, in Croktile, `s32` can not be used alone to define programming entities. For example:
...
Since Croktile handles storage in a heterogenous context, a buffer definition without a storage specifier defaults to the storage type of the host program, i.e., CPU memory. Other storage specifiers are defined by the target. For example, *Cuda/Cute* (for GPU hardware) supports:
...
A *spanned data* is passed between host and tileflow programs. Therefore, a Croktile function can have *spanned data*s as its parameters. The following code showcases an example:
...
The lifetime of storage allocated inside a *Croktile function* is managed by the Croktile compiler for efficient use. The compiler attempts to reuse buffers as much as possible if their lifetimes do not overlap. Therefore users of Croktile are not necessary to do buffer management.

## Quick Summary
This section covered defining and managing data/buffers in Croktile tileflow programs, including declaration syntax, initialization, storage specifiers, and efficient buffer lifetime management by the compiler. This enables us to step further to the next essential topic: the dynamic shape support.
