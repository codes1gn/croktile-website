---
title: "Thread masking"
description: "inthreads, divergence, and asynchronous inthreads in SPMD code."
order: 12
---

## Overview
While Croktile primarily targets SPMD (Single-Program-Multiple-Data) programming, it also supports MPMD (Multiple-Program-Multiple-Data) execution through thread masking. In this section, we demonstrate how to write divergent code using `inthreads` block in Croktile.
...
In Croktile, we are able to create divergent code via `inthreads` block. Let us dive into the detail.

## `inthreads`: Divergent Code with the Thread Mask
### Parallel Perspective of Tileflow Programs
In Croktile, a *divergent* code region is generated with the keyword `inthreads`, where a compare expression followed.
...
You may now re-examine Croktile tileflow programs from the perspective of parallelism. It consists of
...
In Croktile, the `inthreads` block looks like a `if` conditional block in C/C++. However, `inthreads` explicit claims the code divergence, which is a concept in SPMD program, rather than the conditional block for sequential code. Therefore, the `inthreads` block:
...
Croktile compiler has the checks for both the comparison expression and `inthreads`-block placement.
...
## Implicit Masking
In Croktile, code between multiple `parallel-by` levels can be considered as a type of implicit masking. The below code showcases an example:
