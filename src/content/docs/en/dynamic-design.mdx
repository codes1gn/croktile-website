...
description: "Anonymous and symbolic dimensions for runtime-shaped tensors."
order: 9
---
...
void bar(int * data, int dim0) {
  foo(croktile::make_spanview<3>(data, {dim0, 1, 2}));
}
```
...
Another approach in Croktile to support dynamic shape is to use **Symbolic Dimension**. *Symbolic dimension*s are named but as well unknown at compile-time. Here's an example:
...
```cpp
void __croktile_transpiled_Matmul(croktile::span_view<2, croktile::s32> lhs,
                                croktile::span_view<2, croktile::s32> rhs) {
  croktile_assert(lhs.shape()[0] / 32 > 0,
...
void __croktile_transpiled_Matmul(croktile::span_view<2, croktile::s32> lhs,
  ...
  croktile_assert(lhs.shape()[1] == rhs.shape()[0],
